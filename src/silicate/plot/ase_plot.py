from .ase_plot_utils import PlottingVariables, make_patch_list
...
class Matplotlib(PlottingVariables):
    ...
    def write_body(self):
        ...
        if self.atoms_labels is not None:
            for key, value in self.atoms_labels.items():
                x, y = self.positions[key, :2]
                self.ax.text(x-0.1, y-0.1, value, color='black', fontweight='bold', zorder=100)
                #self.ax.text(x-0.3, y-0.3, value, color='red', fontweight='bold', zorder=100)
...
def plot_atoms(atoms, ax=None, return_object=False, **parameters):
    ...
    if ax is None:
        ax = plt.gca()
    m = Matplotlib(atoms, ax, **parameters)
    m.write()
